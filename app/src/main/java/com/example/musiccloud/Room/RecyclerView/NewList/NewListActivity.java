...
public class NewListActivity extends AppCompatActivity {
    ...
    //  适配器
    private NewViewAdapter newViewAdapter;
...
//        初始化控件
//        通过id绑定新歌的RecyclerView组件
        NewRecyclerView = (RecyclerView) findViewById(R.id.new_recycler);
//        调用API，获取新歌榜的歌曲数据
        ...
        new Thread(new Runnable(){
            ...
            public void run(){
                try{
                    ...
                    /**
                     * 调用网易云NodeJS的接口，获取歌单id为3779629的详情信息,新歌榜
                     * 下面是调用的接口链接，这里写了两个，因为多次调用可能会引起安全验证，所以要这里准备了几个接口备用
                     * 如果一个链接调用之后没有接受到数据，将下面的request的url更换另一个api链接
                     * https://music-cloud-fpgfem8aa-aliveleqi.vercel.app/playlist/detail?id=3779629
                     * 或者是
                     * http://duoduozuikeail.top:3000/playlist/detail?id=3779629
                     * https://lianghj.top:3000/playlist/detail?id=3779629
                     * https://yxcr-music-api.vercel.app/detail?id=3779629
                     * */
                    Request request = new Request.Builder()
                            .url("http://duoduozuikeail.top:3000/playlist/detail?id=3779629") //这里要是请求不到数据，换个链接，上面注释里
                            .build();
    ...
    private void showResponse(){
        runOnUiThread(new Runnable(){
            ...
            public void run(){
                ...
                //  创建适配器对象
                newViewAdapter = new NewViewAdapter(songsList);
                //  Recyclerview组件对象设置适配器对象，显示Recyclerview组件
                NewRecyclerView.setAdapter(newViewAdapter);
            }
        });
    }
